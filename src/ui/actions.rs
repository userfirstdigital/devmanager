...
use crate::ui::components::{
    AccessibilityMetadata, AccessibleRole, FocusEpoch, InteractionStateModel,
};

use gpui::KeyBinding;

// The client catalog remains the only source of stable host/task action IDs.
// These GPUI types are the single native dispatch adapters for that catalog;
// they do not create a second transport or command registry.
...
// Local keyboard intents are also declared once here so preview and the real
// native shell cannot drift into separate shortcut/action universes.
#[derive(Clone, Debug, Default, PartialEq, Eq, gpui::Action)]
...
/// Bind the GPUI adapters for all shared client-catalog actions.
...
/// Bind the local keyboard model through the same native action registry.
