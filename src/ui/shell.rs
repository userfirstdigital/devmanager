...
impl Shell {
    ...
    /// Apply a newer host projection's local selection without synthesizing a
    /// task/provider event. A changed selection is a lifecycle boundary and
    /// invalidates any terminal pointer owner before advancing the epoch.
    pub fn sync_selected_task(&mut self, selected_task: Option<TaskId>) -> bool {
        if self.selected_task == selected_task {
            return false;
        }
