...
use crate::client::action::{
    self, ActionDescriptor, TaskCreateArguments, TaskRenameArguments, ACTION_HOST_ACTIONS,
    ACTION_HOST_STATUS, ACTION_TASK_CREATE, ACTION_TASK_LIST, ACTION_TASK_RENAME, ACTION_TASK_SHOW,
};
...
/// Presentational request backed by one closed client catalog entry.
///
/// This adapter deliberately remains inside the native UI boundary: it carries
/// no transport identity and cannot mutate a host. The client action catalog
/// remains the sole source of stable IDs and command/query factories.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ActionRequest {
    HostActions,
    HostStatus,
    TaskList,
    TaskShow { task_id: crate::domain::id::TaskId },
...
impl ActionRequest {
    ...
    pub fn descriptor(&self) -> &'static ActionDescriptor {
        action::catalog()
            ...
            .expect("every UI action request must map to the client catalog")
    }
}
...
impl Add<u64> for FocusEpoch {
    type Output = Self;

    fn add(self, sequence_delta: u64) -> Self::Output {
        Self {
            source_id: self.source_id,
            sequence: self
                .sequence
                .checked_add(sequence_delta)
                .expect("focus epoch sequence exhausted"),
        }
    }
}
