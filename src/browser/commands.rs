...
mod secure_command_tests {
    ...
    async fn controller_view_initialization_retries_share_one_bounded_deadline() {
        ...
        let controller = bridge.bind(workspace_key, Duration::from_millis(500));
        ...
        let result = tokio::time::timeout(Duration::from_secs(2), async {
            loop {
                tokio::select! {
                    joined = &mut task => break joined.expect("controller task"),
                    request = inbox.recv() => {
                        let request = request.expect("initialization retry request");
                        attempts += 1;
        ...
        .await
        .expect("initialization retries must remain bounded by one deadline");

        assert!(attempts >= 2, "the controller should retry initialization");
        assert_eq!(
            result,
