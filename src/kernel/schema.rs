...
// V13 is appended after the Phase 4 provider migrations (V7-V9) and the
// prompt base/corrections/lineage migrations (V10-V12). V12's
// `prompt_lineage_migration_commitment.migration_version = 12` check makes
// folding history into V10 a lineage rewrite, not a safe in-place extension.
const PROMPT_V13_SQL: &str = "\
...
pub(crate) const V14_SHA256: [u8; 32] = [
    0xf5, 0x7e, 0xe3, 0x0a, 0xdc, 0x93, 0xa9, 0x62, 0x85, 0xa0, 0xe2, 0xfe, 0xec, 0x06, 0xd9, 0x35,
    0xc8, 0x16, 0x79, 0x3f, 0x81, 0x1b, 0x59, 0x9f, 0xb7, 0x15, 0xe1, 0x23, 0xbb, 0x29, 0x5d, 0xf8,
];
pub(crate) const V14_SHA256_HEX: &str =
    "f57ee30adc93a96285a0e2feec06d935c816793f811b599fb715e123bb295df8";
        ...
        .get_or_init(|| {
            ...
            let migrations = vec![
                ...
                Migration {
                    ...
                    sql: PROMPT_V10_SQL,
                    sha256: sha256_bytes(PROMPT_V10_SQL),
                },
                Migration {
                    version: 11,
                    name: "phase07-prompts-corrections-v2",
                    sql: PROMPT_V11_SQL,
                    sha256: sha256_bytes(PROMPT_V11_SQL),
                },
                Migration {
                    version: 12,
                    name: "phase07-prompts-lineage-authority-v3",
                    sql: PROMPT_V12_SQL,
                    sha256: sha256_bytes(PROMPT_V12_SQL),
                },
                Migration {
                    version: 13,
...
mod tests {
    ...
    fn schema_v2_upgrades_to_v3_with_explicit_zero_prune_boundary() {
        ...
        assert_eq!(history[9].1, "phase07-prompts-v1");
        assert_eq!(history[9].2.len(), 32);
        assert_eq!(history[10].0, 11);
        assert_eq!(history[10].1, "phase07-prompts-corrections-v2");
        assert_eq!(history[10].2.len(), 32);
        assert_eq!(history[11].0, 12);
        assert_eq!(history[11].1, "phase07-prompts-lineage-authority-v3");
        assert_eq!(history[11].2.len(), 32);
    ...
    fn phase07_prompts_v1_cannot_absorb_history_after_v8_v9() {
        ...
        assert_eq!(migrations[9].name, "phase07-prompts-v1");
        assert_eq!(migrations[10].name, "phase07-prompts-corrections-v2");
        assert_eq!(migrations[11].name, "phase07-prompts-lineage-authority-v3");
        assert!(
            PROMPT_V12_SQL
    ...
    #[test]
    fn v13_upgrade_with_existing_saved_starts_dirty_and_enqueued() {
        ...
        ];
        {
            ...
            for migration in migration_manifest().iter().take(10) {
    ...
    #[test]
    fn v13_upgrade_with_1025_existing_saved_caps_pending_and_sets_overflow() {
    ...
    fn schema_upgrade_matrix_reaches_corrections_before_prompt_lifecycle() {
        for prior_version in 10_i64..=12 {
            let dir = TempDir::new().expect("tempdir");
