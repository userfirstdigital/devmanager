...
fn pending_transition_from_wire(
    ...
    Ok(PendingIdentityTransition {
        ...
        claim_owner: wire.claim_owner.as_deref().map(parse_nonce).transpose()?,
...
fn validate_pending_transition(pending: &PendingIdentityTransition) -> Result<(), IdentityError> {
    ...
    if pending.claim_owner.is_some()
        != (pending.claim_expires_at_epoch_ms.is_some() && pending.claim_logical_revision.is_some())
    {
        return Err(IdentityError::Corrupt);
    }
    if pending
...
fn revocation_journal_from_wire(
    ...
    let entries = wire
        .entries
        .into_iter()
        .map(|entry| Ok((DeviceId::parse(&entry.device_id)?, entry.revocation_epoch)))
...
fn validate_revocation_journal(journal: &PendingRevocationJournal) -> Result<(), IdentityError> {
