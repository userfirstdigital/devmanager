...
use crate::connect::{
    ...
    BrowserPrivateStorage, ConnectRole, CredentialLocation, CredentialVault,
    DeviceEstablishmentHandle, DeviceId, DeviceKeyProof, DeviceKind, DeviceRepairHandle,
    HostEstablishmentHandle, HostKeyProof, HostPublicId, HostRotationHandle, IdentityCommand,
    IdentityError, IdentityLimitField, IdentityOp, IdentityPersistence,
    ...
    RepairDevice, MAX_IDENTITY_ARRAY_ITEMS, MAX_IDENTITY_DEVICES, MAX_IDENTITY_MAP_ENTRIES,
    MAX_IDENTITY_NESTING, MAX_IDENTITY_PHYSICAL_BYTES, MAX_LABEL_BYTES,
};
...
impl CredentialVault for FakeVault {
    ...
    fn recover_host_establishment(
        ...
        if let Some(handle) = self.pending_host_establishment.as_ref().filter(|handle| {
            handle.host_public_id() == host_id && handle.transition_nonce() == transition_nonce
        }) {
    ...
    fn prepare_host_rotation(
        ...
        let proof = HostKeyProof::from_parts_for_test(host_id, generation, fingerprint);
    ...
    fn recover_host_rotation(
        ...
        if let Some(handle) = self.pending_host_handle.as_ref().filter(|handle| {
            handle.host_public_id() == host_id && handle.transition_nonce() == transition_nonce
        }) {
            return Ok(Some(handle.clone()));
        }
    ...
    fn device_repair_committed(&self, handle: &DeviceRepairHandle) -> Result<bool, IdentityError> {
        ...
        self.verify_device(handle.device_id(), handle.proof())
            .map(|_| true)
    }
    ...
    fn rollback_device_repair(&mut self, handle: &DeviceRepairHandle) -> Result<(), IdentityError> {
        ...
        if let Some(previous) = self.pending_device_repairs.remove(&handle.device_id()) {
            self.devices.insert(handle.device_id(), previous.previous);
        }
        Ok(())
    }
...
fn store_binding_and_authoritative_permission_reject_stale_revoked_proof() {
    ...
    assert_eq!(
        bind_device_credential(&identity, &harness.binding, &harness.vault, device_id, 9,)
