...
impl HostMembership {
    ...
    pub fn confirm_locally(&mut self, now_ms: i64, policy: &OrganizationPolicyDocument) -> Result<(), OrgError> {
        if self.status == MembershipStatus::Revoked {
            return Err(OrgError::MembershipRevoked);
        }
        if self.policy_revision != policy.revision || self.policy_hash_hex != policy.content_hash_hex
        {
            return Err(OrgError::StalePolicy);
        }
