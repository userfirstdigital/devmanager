    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::StandaloneMode => {
                write!(
                    f,
                    "organization features are unavailable in anonymous local mode"
                )
            }
            ...
            Self::UncertainOutcome => {
                write!(
                    f,
                    "ambiguous local apply remains uncertain without automatic repeat"
                )
            }
            ...
            Self::ReviewRequired => {
                write!(f, "EvidenceBundle must be reviewed before Task creation")
            }
