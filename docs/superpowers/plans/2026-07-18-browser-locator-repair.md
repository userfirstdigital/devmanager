...
- Evidence is owner-scoped and pinned only while the exact unresolved repair owns it.
...
2. Write failing real-store tests for exact owner/kind validation, immediate pinning, second-pin rollback, pin retention while paused, and unpin on cancel/replace/terminal/drop.
3. Add one private repair state to `ActiveBrowserReplay`, one private evidence pin lease, one private override map, and a value-free Tokio watch generation shared with the execution handle. Remove the unrestricted placeholder resume transition; only a confirmed apply may resume.
4. Keep repair values non-Debug/non-serde; projection contains only IDs, slot, revision, phase, tab ID, and handles.
5. Run replay/repair/resources tests; commit `feat(browser): add exact replay repair state`.
...
2. Require this exact sequence after failure: semantic snapshot, verify/pin, viewport screenshot, verify/pin, exact coordinator pause. No later recipe command may be issued while paused.
3. Implement locator-slot mapping for primary/optional/source/destination/action-wait/step-wait/assertion. Preserve absent/hidden semantics that intentionally succeed without an element.
4. Keep `execute_browser_replay` alive on the watch receiver. Prove cancel, replace, and workspace interruption wake it, return the retained terminal projection, close secrets, ignore late responses, and unpin evidence. Capture/pin failure remains `StepFailed` with rollback.
...
1. Write failing Node/host tests for a pointer-transparent owned highlight that dispatches no focus/click/input events, is excluded from DOM revision tracking, replaces/clears exactly, and returns typed missing/stale/unavailable errors.
2. Write failing authority tests for exact repair instance, candidate `BrowserElementRef` revision, semantic locator validation, User/Agent actors, late callbacks, and changed page/workspace/replay/repair.
3. Add internal highlight/clear commands through the existing queue and journal, explicitly excluded from recording. Store a candidate only after exact host acknowledgement and post-await authority recheck.
...
1. Write failing tests for canonical recipe digest, exact step index+ID+slot+old-locator comparison, every locator slot, changed recipe, invalid candidate, canonical-root/reparse boundaries, concurrent apply, injected replacement failure, and temp cleanup.
2. Store the validated canonical digest privately in the replay plan. Add locator-at/replace-at helpers and one process-local repair write gate.
...
2. Write failing executor tests for exact override slot, no progress advance before retry, applied-without-resume, later idempotent resume, successful same-step advance, second repair ID on another failure, and cleanup.
3. Add one exact apply reservation. After successful atomic write, install the private override; resume only the exact paused instance and wake its existing executor. Never mutate the immutable plan or another locator slot.
4. Run all replay/repair/host/recording/secret suites; commit `feat(browser): resume repaired replay steps`.
