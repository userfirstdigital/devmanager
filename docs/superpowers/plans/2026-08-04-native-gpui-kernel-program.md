...
- **Herdr:** Phases 2–4 selectively adopt its Rust server-owned PTY/client split, one replaceable pending render frame per slow client, observer/controller separation, cached Windows process-attribution helpers, provider resume tables, detection manifests, and one lifecycle authority per pane. Keep DevManager's stronger Job Object ownership, bounded MessagePack protocol, native GPUI cockpit, invisible solo-device handoff, and Connect relay; do not copy Herdr's TUI, newline-JSON transport, SSH product boundary, or PID-tree termination model. Any copied Apache-2.0/MIT code retains upstream notices and marked modifications.
