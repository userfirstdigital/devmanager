...
- [x] **Step 1: Add failing store tests** for an empty migration, WAL mode, foreign-key enforcement, newer/changed/gapped migration rejection, projection rebuild, primary-agent validation, resource-generation fencing, corrupt/truncated database, interrupted transaction, integrity-check failure, codec/column mismatch, and integer overflow. Assert exact table/index names with `sqlite_schema`.
- [x] **Step 2: Run** `cargo test --test kernel_store schema_ -- --nocapture` and observe the missing schema module.
- [x] **Step 3: Implement** an immutable, compiled migration manifest whose `(version, name, sha256)` entries are contiguous and exact. Apply only the next migration inside a transaction; reject a newer database, a changed migration hash/name, or a gapped history. Open file databases with WAL, `foreign_keys=ON`, a bounded busy timeout, and `synchronous=NORMAL`; use full durability for explicit checkpoint/export operations.
- [x] **Step 4: Keep the public store opaque:** `KernelStore::open(&Path) -> Result<Self, StoreError>` and `rebuild_projections(&mut self) -> Result<ProjectionRebuild, StoreError>`, with schema/projector modules crate-private and no public `rusqlite` types. Store identifiers as fixed 16-byte blobs, timestamps as Unix milliseconds, only the inner event payload as MessagePack (the `events` columns are the sole durable envelope), and human-queryable discriminants as stable text. Verify decoded payload type/version against its columns. Use checked `u64` to non-negative SQLite `i64` conversions everywhere and return a typed range error; never use `as`. Add indexes for task sequence, task revision, operation state, outbox delivery state, operation/effect identity, and active resources. V1 keeps compact command receipts for permanent idempotency; it does not expire a known `CommandId` into a second mutation.
- [x] **Step 5: Implement projectors** for `tasks`, `operations`, `agent_sessions`, `artifacts`, and `resources`. Projection functions accept a transaction and one event and contain no clocks, random generation, filesystem calls, or network calls. Derive `operations.task_id` only from `DomainEvent.task_id`; persist the accepted event's paired `resource_id`/`runtime_generation`, and require outcomes to match stored command/task/resource/action-epoch/generation fences. Validate `tasks.primary_agent_session_id` in the projector instead of a cyclic foreign key: the selected session must exist, belong to that task, and have role `Primary`. Resource release projections must match the current generation and use `DomainEvent.occurred_at_ms` for `updated_at_ms`.
- [x] **Step 6: Implement `rebuild_projections`** with temporary shadow projection tables and deterministic ordered event replay. Compare canonical rows, then atomically replace the contents of the stable projection tables in one transaction; do not rename tables or touch `events`, `command_receipts`, or `outbox`. This is a repair/verification tool, not normal startup work.
- [x] **Step 7: Run** `cargo test --test kernel_store schema_ -- --nocapture`; commit as `feat(kernel): add sqlite event schema and projections`.

### Task 1.4: Make command execution atomic and idempotent

**Files:** `src/kernel/{store,command_bus,outbox}.rs`, `tests/kernel_store.rs`

**Contract:** `KernelStore::execute(CommandEnvelope) -> CommandReceipt` performs lookup, revision validation, operation allocation, decision, append, projection, receipt, and outbox insert in one immediate transaction. Side-effect callbacks cross the store boundary only through an opaque current `DispatchPermit` or `ReconciliationClaim`; their separate outcome transaction remains idempotent and generation/action-epoch/resource-fenced, but an unclaimed or superseded worker cannot terminalize an operation through a bare public outcome call.

**Lean execution slices:** (a) domain/store contracts and strict V1 codecs; (b) pure command path and permanent receipts; (c) side-effect acceptance and stable outbox identity; (d) outcome/reconciliation transaction core; (e1) additive V2 plus claim/begin/renew/release fencing; (e2a) ambiguity recovery, reconciliation claims, and the one-use absence receipt; (e2b) permit-bound terminal callbacks plus complete removal of the bare public outcome seam; (f) rollback/concurrency proof and one focused gate. e2a/e2b are reviewed work batches in one final e2 contract commit: no compatibility wrapper, test-only public backdoor, or dual API may remain. Finish and review each slice before starting the next; no slice may expose SQLite or implement provider/process control.

Dispatch and reconciliation scans must continue past stale-fenced rows so valid work is not starved, but may not silently report a clean queue when only stale work remains. They surface `StaleFence`; Task 1.4f proves operator-visible quarantine/terminalization. Retry delays are scheduling hints rather than outcome identity: an exact same-generation callback replay returns the already committed result without rescheduling.

Phase 1 has no real effect whose replay policy is `NoAutomaticRetry`, so this task locks its conservative policy classification without manufacturing an unreachable durable/provider path. Phase 4 activates and proves the `Uncertain` transition with the first genuine non-retryable provider effect.

Safe dispatch/reconciliation callback APIs do not accept durable event timestamps from providers. The store owns commit and observation time from its durable clock floor, preventing a worker from backdating or future-dating kernel history; exact callback replay reads the committed time from durable state.

`OperationOutcome` carries operation ID, observed time, optional action epoch, one paired optional resource fence, outcome kind, and `OutcomeSource::{Dispatch, VerifiedReconciliation { effect_index, external_identity }}`. It does not duplicate `command_id`; the store derives that one truth from the accepted operation when it writes the durable outcome fact. Settled outcomes carry result event IDs. Verified reconciliation may resolve only `Uncertain -> Settled/Failed`, and the source/evidence must be represented in the durable fact so replay can enforce the same transition.

- [x] **Step 1: Add failing tests** `accepted_command_commits_operation_and_all_records`, `accepted_side_effect_remains_accepted_until_outcome`, `pure_database_command_settles_in_acceptance_transaction`, `projector_failure_rolls_back_every_record`, `duplicate_command_returns_original_operation`, `revision_conflict_persists_only_rejection_receipt`, `duplicate_rejected_command_stays_rejected`, `stale_generation_cannot_record_outcome`, `duplicate_outcome_returns_original_state`, `retry_safe_effect_reuses_external_idempotency_key`, `reconcile_before_retry_proves_absence`, `ambiguous_non_retryable_effect_becomes_uncertain`, `uncertain_effect_is_not_auto_dispatched_again`, and `concurrent_writers_accept_only_one_revision`.
- [x] **Step 2: Run** `cargo test --test kernel_store command_ -- --nocapture` and save the red output.
- [x] **Step 3: Implement `KernelStore`** with a single writer connection owned by the kernel executor, retain its canonical database path, and open private read-only/query-only snapshot connections from that path. No caller receives a raw writable `rusqlite::Connection`.
- [x] **Step 4: Start an immediate transaction**, return an existing receipt/operation before decision when `command_id` is known, load the current projection, compare `expected_task_revision`, and decide. Derive effective task scope from `CreateTaskIntent.id` when the create envelope is intentionally unscoped. Persist a final rejection receipt without operation/events/projection/outbox; the same CommandId can never become accepted on retry.
- [x] **Step 5: For acceptance, allocate all IDs, write the receipt and its deferred-linked `Accepted` operation row, allocate event sequence, apply projections, and insert one outbox row per required side effect using stable `(operation_id, effect_index)` identity plus an explicit replay policy**, then commit. Keep `decide` authoritative and pure; a separate crate-private planner derives effects from accepted events plus the pre-command snapshot. Store strict versioned MessagePack receipt/effect documents and verify decoded policy/destination against their queryable columns. Pure database commands also write their `OperationSettled` fact in this transaction. Convert constraint, corruption, busy, stale-fence, conflicting-outcome, and missing-operation failures into distinct `StoreError` variants; never claim settlement merely because acceptance committed.
- [x] **Step 6: Implement dispatch/outcome rules.** Add an immutable V2 migration with a monotonic lease generation and strict attempt-bound reconciliation receipt. The exact durable outbox paths are `pending -> claimed -> dispatching`; accepted-operation reconciliation uses `dispatching -> reconcile_required -> reconciling -> pending | reconcile_required | settled | failed`; uncertain-operation reconciliation uses `uncertain -> reconciling -> uncertain | settled | failed`; and `settled | failed | cancelled` are final. Only the declared transitions are accepted. `RetrySafe` reuses the same versioned external idempotency key; `ReconcileBeforeRetry` leaves the operation `Accepted` while absence can authorize exactly one retry, while verified-present reconciliation records `Uncertain` immediately before its verified final resolution; `NoAutomaticRetry` records dispatch start before crossing the external boundary and becomes `Uncertain` after an ambiguous crash/timeout. Only a current permit/claim with the exact lease generation and current operation command/task/action epoch/resource/runtime generation may record a callback. A verified reconciliation fact may resolve `Uncertain` to `Settled`/`Failed`, but never re-dispatches it implicitly. Duplicate identical outcomes are idempotent and conflicting/stale outcomes are rejected with evidence.
- [x] **Step 7: Add a deterministic rollback proof using a test-installed SQLite trigger** between event insertion and projection; do not ship a production failpoint API.
- [x] **Step 8: Run** the focused command tests and complete `kernel_store` gate; commit the final proof as `test(kernel): prove atomic command serialization`.
