...
- [ ] **Step 3: Implement `KernelStore`** with a single writer connection owned by the kernel executor, retain its canonical database path, and open private read-only/query-only snapshot connections from that path. No caller receives a raw writable `rusqlite::Connection`.
...
- [ ] **Step 7: Add a deterministic rollback proof using a test-installed SQLite trigger** between event insertion and projection; do not ship a production failpoint API.
