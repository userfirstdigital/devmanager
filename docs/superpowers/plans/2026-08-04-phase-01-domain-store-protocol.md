...
- [ ] **Step 6: Make event serialization explicit** with `{ event_type, schema_version, payload }`; include `OperationSettled`, `OperationFailed`, `OperationCancelled`, and `OperationUncertain` facts correlated to command/operation IDs and generation/action epoch. Add golden JSON fixtures so renaming a Rust enum cannot silently change the durable format.
