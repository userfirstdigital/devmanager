...
**Lean execution slices:** (a) domain/store contracts and strict V1 codecs; (b) pure command path and permanent receipts; (c) side-effect acceptance and stable outbox identity; (d) outcome/reconciliation transaction; (e) dispatch policies; (f) rollback/concurrency proof and one focused gate. Finish and review each slice before starting the next; no slice may expose SQLite or implement provider/process control.
...
- [ ] **Step 6: Implement dispatch/outcome rules.** `RetrySafe` reuses the same external idempotency key; `ReconcileBeforeRetry` may retry only after stable external identity proves the effect absent; `NoAutomaticRetry` records dispatch start before crossing the external boundary and becomes `Uncertain` after an ambiguous crash/timeout. Only the current operation command/task/action epoch/resource/runtime generation may move `Accepted` to `Settled`, `Failed`, `Cancelled`, or `Uncertain`; a verified reconciliation fact may later resolve `Uncertain` to `Settled`/`Failed`, but never re-dispatches it implicitly. Duplicate identical outcomes are idempotent and conflicting/stale outcomes are rejected with evidence.
