...
- A plain shell's `ReleaseResource` row is settled process-empty by the host itself right after `close_shell_terminal` (client close, orphan sweep) or by maintenance when the session is already gone; the archive loop keeps `settle_next_resource_release`. Before this, the sole claimant of the `ResourceRelease` destination class was that archive loop, so a shell closed any other way stayed `Releasing` with its `terminal_facts` row and its strip entry forever and the client rendered a muted "?" chip that never cleared. `CommandBus::settle_resource_release_for` settles BY RESOURCE, in one IMMEDIATE transaction, stepping over other resources' rows without claiming them: the host tears one shell down at a time and must never settle a release whose PTY it has not closed.
- The orphan sweep also finishes a `Releasing` plain shell whose host link is still alive. Both ordered release paths (the `CloseTerminal` effect and the task-close loop) close the PTY and settle inside one executor step, so a reaper tick can only see `Releasing` with a live link when the release was begun by a bare `Command::ReleaseResource` (or a `CloseTerminal` written by another store handle); the sweep tears the process down first and settles the row after, instead of leaving the PTY running behind a "?" chip until archive.
