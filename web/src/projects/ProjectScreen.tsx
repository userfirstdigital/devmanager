...
export function ProjectScreen({
  ...
  const launch = (kind: "claude" | "codex") => {
    ...
    void launchAiTab(project.id, kind).then(() => {
      ...
      if (key) onNavigate(routeForStableSessionKey(key));
    });
  };
    ...
    <section className="dm-screen" aria-labelledby="project-title">
      ...
      <div className="dm-screen-scroll">
        <section className="dm-launch-grid" aria-label="Start an AI session">
          ...
          >
            <span className="dm-launch-icon dm-claude" aria-hidden="true">
              <Bot size={22} />
            </span>
            <span>
              <strong>New Claude</strong>
              <small>Start coding</small>
            </span>
          </button>
          <button
            type="button"
          ...
          >
            <span className="dm-launch-icon dm-codex" aria-hidden="true">
              <TerminalSquare size={22} />
            </span>
            <span>
              <strong>New Codex</strong>
              <small>Start coding</small>
            </span>
          </button>
        </section>
            ...
            <div className="dm-grouped-list">
              {tabs.map((tab) => {
                ...
                const session = workspace.sessions.find(
                  (candidate) => candidate.stableSessionKey === key,
                );
                return (
                  <button
                    key={tab.id}
                    ...
                    onClick={() => {
                      ...
                      onNavigate(routeForStableSessionKey(key));
                    }}
                  >
                    <span className="dm-row-leading" aria-hidden="true">
                      <Bot size={19} />
                    </span>
                    <span className="dm-row-copy">
                      <strong>
                        {tab.label?.trim() ||
                          `${tab.kind === "claude" ? "Claude" : "Codex"} session`}
                      </strong>
                      <small>{session ? session.status : "Not running"}</small>
                    </span>
                    <ChevronRight
                      className="dm-row-chevron"
                      size={18}
                      aria-hidden="true"
                    />
                  </button>
                ...
                return (
                  <div className="dm-command-row" key={command.id}>
                    <button
                      ...
                      disabled={!session}
                      onClick={() => onNavigate(routeForStableSessionKey(key))}
                    >
                      <span className="dm-row-leading" aria-hidden="true">
                        <Server size={19} />
                      </span>
