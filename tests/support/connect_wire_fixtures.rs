//! Complete native payload set consumed by the fixture generator and contract tests.

use devmanager::connect::{
    native_browser_contract_fixtures, CanonicalSchemaFixture, ConnectPayload,
};
...
pub fn native_browser_wire_fixtures() -> Vec<CanonicalSchemaFixture> {
    let mut fixtures = native_browser_contract_fixtures()
        .into_iter()
        .filter(|fixture| matches!(fixture.payload.kind().get(), 1 | 18 | 19 | 20 | 21 | 22))
        .collect::<Vec<_>>();
    fixtures.extend(receipt_status_fixtures());
    fixtures
}
