...
impl Application {
    ...
    /// Build an X11 application, including on a desktop with XWayland.
    /// This leaves the process environment unchanged for child applications.
    #[cfg(all(any(target_os = "linux", target_os = "freebsd"), feature = "x11"))]
    pub fn new_x11() -> anyhow::Result<Self> {
        Ok(Self(App::new_app(
            crate::platform::x11_platform()?,
            Arc::new(()),
            Arc::new(NullHttpClient),
        )))
    }
