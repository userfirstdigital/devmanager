...
/// Select X11 explicitly without changing the environment inherited by children.
#[cfg(all(any(target_os = "linux", target_os = "freebsd"), feature = "x11"))]
pub(crate) fn x11_platform() -> anyhow::Result<Rc<dyn Platform>> {
    Ok(Rc::new(X11Client::new()?))
}
