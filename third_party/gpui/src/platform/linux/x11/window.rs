...
impl rwh::HasWindowHandle for X11Window {
    fn window_handle(&self) -> Result<rwh::WindowHandle<'_>, rwh::HandleError> {
        let state = self.0.state.try_borrow().map_err(|_| rwh::HandleError::Unavailable)?;
        if state.destroyed {
            return Err(rwh::HandleError::Unavailable);
        }
        let id = NonZeroU32::new(self.0.x_window).ok_or(rwh::HandleError::Unavailable)?;
        let handle = rwh::XcbWindowHandle::new(id);
        // SAFETY: this window owns the XID and its retained XCB connection.
        // X11Window::drop destroys it only after this borrow has ended.
        Ok(unsafe { rwh::WindowHandle::borrow_raw(handle.into()) })
    }
}
impl rwh::HasDisplayHandle for X11Window {
    fn display_handle(&self) -> Result<rwh::DisplayHandle<'_>, rwh::HandleError> {
        ...
        let connection = NonNull::new(connection).ok_or(rwh::HandleError::Unavailable)?;
        let handle = rwh::XcbDisplayHandle::new(Some(connection), self.0.screen_id);
        // SAFETY: the Rc in this window retains the exact connection.
        Ok(unsafe { rwh::DisplayHandle::borrow_raw(handle.into()) })
    }
}
...
impl X11Window {
    pub fn new(
        ...
        let ptr = X11WindowStatePtr {
            ...
            xcb: xcb.clone(),
            x_window,
            screen_id: i32::try_from(x_main_screen_index)?,
        };
