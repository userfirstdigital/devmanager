...
pub trait SlavePty {
    ...
    /// must either consume it with `resume` after recording ownership or abort
    /// it. Implementations without this containment boundary fail closed.
    #[cfg(windows)]
    fn spawn_command_suspended_in_job(
        &self,
        cmd: CommandBuilder,
        job: BorrowedHandle<'_>,
    ) -> Result<win::PendingChild, Error> {
        let _ = (cmd, job);
        Err(anyhow::anyhow!(
            "this Windows PTY implementation cannot spawn suspended in a Job Object"
        ))
    }
}
