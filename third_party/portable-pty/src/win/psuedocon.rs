use super::{PendingChild, WinChild};
...
impl PsuedoCon {
    ...
    pub fn spawn_command(&self, cmd: CommandBuilder) -> anyhow::Result<WinChild> {
        let (child, primary_thread) = self.create_command(cmd, None, false)?;
        drop(primary_thread);
        Ok(child)
    }

    pub fn spawn_command_suspended_in_job(
        &self,
        cmd: CommandBuilder,
        job: BorrowedHandle<'_>,
    ) -> anyhow::Result<PendingChild> {
        let (child, primary_thread) = self.create_command(cmd, Some(job), true)?;
        Ok(PendingChild::new(child, primary_thread))
    }
    ...
    fn create_command(
        ...
        suspended: bool,
    ) -> anyhow::Result<(WinChild, OwnedHandle)> {
        ...
        // Own both handles before returning to any fallible caller work.
